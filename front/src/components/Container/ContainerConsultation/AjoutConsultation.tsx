...
import { getAllRendezVous, RendezVous } from '../../../services/rendezVous_api';
...
const AjoutConsultation = () => {
  ...
  const [formData, setFormData] = useState<Omit<Consultation, 'dateHeure' | 'cinPatient' | 'cinPraticien'> & { 
    dateConsult: string 
  }>({
    idConsult: 0,
    idRdv: 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const consultationData: Consultation = {
        ...formData,
        dateHeure: formData.dateConsult,
        cinPatient: '',
        cinPraticien: ''
      };
                    ...
                    <div className="input-field-div">
                      <label>Rendez-vous</label>
                        ...
                        {rendezVousList.map(rdv => {
                            const dateFormatee = new Date(rdv.dateHeure).toLocaleString('fr-FR', {
                            day: 'numeric',
                            month: 'numeric',
                            year: 'numeric',
                            hour: '2-digit',
                            ...
                            const nomPraticien = rdv.nomPraticien || `Dr. ${rdv.cinPraticien}`;

                            return (
                            <option key={rdv.idRdv} value={rdv.idRdv}>
                               Rendez-vous avec Docteur {`${nomPraticien}`} le {`${dateFormatee}`}
                            </option>
                            );
                        })}
                        </select>
